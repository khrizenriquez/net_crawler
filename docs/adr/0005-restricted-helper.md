...
Write active channel-wide captures as `.pcap.partial`. An internal root-owned supervisor publishes the completed `.pcap` with an atomic rename after `tcpdump` exits naturally or after a safe `stop`. The worker ignores partial files and accepts only published `.pcap` staging artifacts.
