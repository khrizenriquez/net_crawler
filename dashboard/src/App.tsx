...
function Overview({ data, refresh }: { data: Data; refresh: () => Promise<void> }) {
  ...
  <section className="overview-grid lower"><Panel title="Radios autorizadas" tag="BSSID CONFIRMADOS"><RadioList radios={data.radios} /></Panel><Panel title="Control manual" tag="HELPER LIMITADO"><div className="control"><p>La captura local recopila tráfico real únicamente de esta Mac y se marca como cobertura parcial. La captura de radio requiere monitor mode compatible.</p><div><button onClick={() => void api.startLocal().then(refresh)}>Capturar tráfico local</button><button className="ghost" onClick={() => void api.start(data.radios[0]?.channel ?? 2).then(refresh)}>Capturar radio</button><button className="ghost" onClick={() => void api.stop().then(refresh)}>Detener</button></div></div></Panel></section></>;
}
